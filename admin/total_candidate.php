...
if(!isset($_SESSION['super_admin'])) {   
  ...
  $user_id = $_SESSION['super_admin'];
    ...
    $total_applied = "DELETE FROM my_profile WHERE Main_id =$id ";  

      if(mysqli_query($conn, $total_applied)){

      header("location: total_candidate");
  ...
  $pagesql  ="SELECT * FROM my_profile";
  ...
  $total_candidate = $totalres ;

  // Total Cadndidate Show....

  $user_quary = "SELECT * FROM  my_profile ORDER BY Main_id LIMIT $start_from, $per_page ";
 ...
 <?php  include('menu.php') ?>
  ...
  <div class="content-wrapper"> 
    ...
    <section class="content-header sty-one">
      <h1>Total Candidate <?php echo $total_candidate ; ?> </h1>
      <ol class="breadcrumb">
        <li><a href="index">Home</a></li>
		<li class="sub-bread"><i class="fa fa-angle-right"></i> Settings</li>
        <li><i class="fa fa-angle-right"></i> Total Candidate</li>
      </ol>
    </section>
...
$degree_quarry = "SELECT * FROM my_resume WHERE  Main_id = '$id' AND Category= 'Education'   ";
$degree_result = mysqli_query($conn, $degree_quarry);
$degree_row = mysqli_fetch_array($degree_result);



$apply_quarry = "SELECT * FROM applied_job WHERE  my_profile_id = $id ";
$apply_result = mysqli_query($conn, $apply_quarry);
$apply_count  = mysqli_num_rows($apply_result);

  ?>

                      <tr>
                        <td><?php echo $id; ?> </td>
                        <td><img src="../photo/<?php echo $user_row ['Photo']; ?>" class="img-circle img-w-30" alt="User Image"> <a href="../profile_view?id=<?php echo $id; ?> " target= "_blank"><?php echo $user_row ['First_Name']." ".$user_row ['Last_Name']; ?></a></td>
                        <td><?php echo $user_row ['Email'] ; ?></td>
                        <td><?php echo $degree_row ['Tittle'] ; ?></td>
                        <td><?php echo $apply_count ; ?> Jobs</td>
                        <td> <a href="total_candidate?action=delete&id=<?php echo $user_row['Main_id']  ?>" onclick="return confirm('Are you sure you want to delete this item?'); "><button type="button" class="btn-danger"><i class="fa fa-trash" aria-hidden="true"></i></button></a> </td>
